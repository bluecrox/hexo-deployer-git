...
module.exports = function(args) {
  ...
  var subDir = 'blog'; //should not hard code
  var deploySubDir = pathFn.join(deployDir, subDir);
  ...
  function push(repo) {
    ...
    return git('add', addOptions).then(function() {
  ...
  return fs.exists(deployDir).then(function(exist) {
    ...
    fs.mkdirsSync(deploySubDir);

    return fs.copyDir(publicDir, deploySubDir, opts);
  }).then(function() {
    ...
    var mapFn = function(dir) {
      ...
      var extendDist = pathFn.join(deploySubDir, dir);
